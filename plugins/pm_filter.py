...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("🤒 𝐇𝐞𝐲 𝐅𝐫𝐢𝐞𝐧𝐝, 𝐓𝐡𝐢𝐬 𝐈𝐬 𝐑𝐞𝐪𝐮𝐞𝐬𝐭𝐞𝐝 𝐁𝐲 𝐎𝐭𝐡𝐞𝐫𝐬.𝐏𝐥𝐞𝐚𝐬𝐞 𝐑𝐞𝐪𝐮𝐞𝐬𝐭 𝐘𝐨𝐮𝐫𝐬𝐞𝐥𝐟...🙂", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def advantage_spoll_choker(bot, query):
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('𝚃𝙷𝙸𝚂 𝙼𝙾𝚅𝙸𝙴 I𝚂 𝙽𝙾𝚃 𝚈𝙴𝚃 𝚁𝙴𝙻𝙴𝙰𝚂𝙴𝙳 𝙾𝚁 𝙰𝙳𝙳𝙴𝙳 𝚃𝙾 𝙶𝚁𝙾𝚄𝙿,𝚃𝚈𝙿𝙴 𝙲𝙾𝚁𝚁𝙴𝙲𝚃 𝚂𝙿𝙴𝙻𝙻 💌')
...
async def advantage_spell_chok(msg):
    ...
    await msg.reply(" 𝗥𝗲𝗮𝗱: <a href=https://t.me/rulesandregulation/6><b></b>ɢʀᴏᴜᴘ ꜱᴇᴀʀᴄʜ ʀᴜʟᴇꜱ</a> \n I couldn't find anything related to that. \nDid you mean any one of these?",
                    reply_markup=InlineKeyboardMarkup(btn))
